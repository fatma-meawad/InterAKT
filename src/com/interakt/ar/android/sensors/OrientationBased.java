...
public class OrientationBased {
	...
	public void start(Context context) {
					...
					if (!quit) {
						...
						if (type == Sensor.TYPE_MAGNETIC_FIELD) {
							...
							if (markerMode) {
								...
								// Log.d("sensor ", "ACC OLD" + linearAcc[0] +
								// " " +
								// linearAcc[1] +
								// " " + linearAcc[2]);

								if (Math.abs(evt.values[0]) > CHANGE_FACT
										|| Math.abs(evt.values[1]) > CHANGE_FACT
										|| Math.abs(evt.values[2]) > CHANGE_FACT) {
									// Log.d("sensor ", "ACC NOT stable");
									stable = false;
									sTime = System.currentTimeMillis();

								} else if (System.currentTimeMillis() - sTime > 1000) {
									stable = true;
									// Log.d("sensor ", "Acc stable");
								}

								// linearAcc[0] = evt.values[0];
								// linearAcc[1] = evt.values[1];
								// linearAcc[2] = evt.values[2];
						...
						if ((type == Sensor.TYPE_MAGNETIC_FIELD)
							...
							matrix = matT.val;
							lookAt[0] = Math.abs(newLookAt[0] - lookAt[0]) > 30 ? newLookAt[0]
									+ position[0]
									: lookAt[0];
							lookAt[1] = Math.abs(newLookAt[1] - lookAt[1]) > 30 ? newLookAt[1]
									+ position[1]
									: lookAt[1];
		...
		sensorLinAcce = sensorMan
				.getSensorList(Sensor.TYPE_LINEAR_ACCELERATION).get(0);
